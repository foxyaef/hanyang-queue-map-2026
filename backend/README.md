...
- `/`: 사용자 대기 지도
- `/admin`: 대기열 길이와 입장 상태를 수정하는 관리자 화면
...
- `GET /api/v1/admin/queues`: 관리자 인증 및 6개 대기열 조회
...
관리자 화면에서 입력한 비밀번호는 브라우저 저장소에 남기지 않으며, 페이지가 열려 있는 동안 메모리에만 유지합니다. 슬라이더 값은 `0~1000` 정수로 변환되어 저장되고, 입장문의 `입장 시작/입장 마감` 선택은 `isClosed` 값으로 저장됩니다.
